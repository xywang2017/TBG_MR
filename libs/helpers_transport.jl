...
function plot_ρs(transport_coefficients,eBτs,fname)
    ...
    tight_layout()
    # savefig("strained_unsymmetrized_rho_phi4.png",transparent=true)
    # savefig("unstrained_unsymmetrized_rho.png",transparent=true)
    display(fig)
    close(fig)
    return nothing
end
...
function plot_symmetrized_transport(νs,ρ,fname)
    ...
    # xlim([-1,1])
    tight_layout()
    # savefig("symmetrize_rho.png",transparent=true)
    display(fig)
    close(fig)
end
...
function plot_hall(νs,δρ,fname)
    ...
    # xlim([-4,4])
    tight_layout()
    # savefig("strained_rho_21.png",transparent=true)
    display(fig)
    close(fig)
end
...
function plot_ωcτavg_versus_filling(νs,fname)
    ...
    xlabel(L"ν")
    tight_layout()
    savefig("omegactau.png",transparent=true)
    display(fig)
    close(fig)
end
...
function plot_principle_strain_axis(νs,δθ,fname)
    ...
    ylim([0,185])
    tight_layout()
    savefig("principal_axis.png",transparent=true,dpi=600)
    display(fig)
    close(fig)
end
...
function plot_principal_strain_axis_vs_unit_cell(iν::Int)
    ...
    axis("equal")
    tight_layout()
    savefig("eps02_$(ν).png",transparent=true,dpi=600)
    display(fig)
    close(fig)
    return nothing 
end
...
function plot_transport_filling(νs,ρ,flag,ν)
    ...
    title("ν=$(ν)")
    tight_layout()
    savefig("nu0.5_unsymmetrized.png",transparent=true)
    display(fig)
    close(fig)
end
...
function plot_uc_vector()
    ...
    ylabel("Moire area")
    tight_layout()
    savefig(joinpath(fpath,"test/moire_area_vs_phi.png"),transparent=true)
    display(fig)
    close(fig)
end
# plot_uc_vector()
...
function dirac_splitting()
    ...
    ylim([0,10])
    tight_layout()
    # savefig(joinpath(fpath,"test/dirac_splitting_upper_bound.png"),transparent=true)
    display(fig)
    close(fig)
    return nothing
...
function plot_contours()
    ...
    # savefig("vanhove_phi$(ϕ).png",transparent=true)
    close(fig)
end
# plot_contours()
...
function get_vanhove_from_dos(Hk,Latt)
    ...
    yticks([])
    tight_layout()
    # savefig("vanhove_schematic.png",transparent=true)
    display(fig)
    close(fig)
    println(Es)
    println(νs)
...
function plot_vanhove(flag="energy")
    ...
    if isequal(flag,"energy")
        ...
        # axvline(60)
        tight_layout()
        savefig("vanhove_energy_vs_phi.png",transparent=true)
    elseif isequal(flag,"filling")
        ...
        # axvline(60)
        tight_layout()
        # savefig("vanhove_filling_vs_phi.png",transparent=true)
    end
    display(fig)
    close(fig)
end
